package usmeni_ispit.qB16_min_distance_2d;
...
public class MinDistance2D {
	...
	private double findDQ(int from, int to) {
		...
		if (to == from)								// u nizu postoji samo jedna tacka ???
			return Double.MAX_VALUE;
		
		if (to - from == 1)
			return points[from].dist(points[to]);	// u nizu postoje dve tacke ali su indeksi obrnuti ???
		
		int median = (from + to) / 2;				// sredina niza
	...
	private double findMinCrossing(int from, int to, int median, double delta) {
		...
		double medianX = points[median].x;			// pozicija vertikale koja deli skup tacaka
		
		Point2D[] box = new Point2D[points.length];	// niz u koji se smestaju tacke na udaljenosti delta od vertikale medianX
		int boxCnt = 0;								// brojac elemenata u Point2D[] box
		
		for (int i = from; i <= to; i++)
			if (Math.abs(points[i].x - medianX) < delta)
				box[boxCnt++] = points[i];			// ako je tacka na udaljenosti po x manjoj do delta od medianX smesta se u niz
		...
		for (int i = 0; i < boxCnt - 1; i++) {
			...
			for (int j = i + 1; j < boxCnt; j++) {
				if (box[j].y - current.y >= delta)	// ako su tacke vertikalno na udaljenosti vecoj
					break;							// od delta dist. ne moze biti minimalna
